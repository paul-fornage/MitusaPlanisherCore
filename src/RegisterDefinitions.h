...
namespace HregAddr {
    enum HregAddr {
        // HREG.  Addresses start at 40,000. (Yes, decimal) Why? Ask Sir Jonathan Modbussy
        // ACTUAL_POSITION_REG_ADDR        = 4002,     // R    actual position of the axis measured in hundredths of an inch
        CC_COMMANDED_POSITION_REG_ADDR  = 40012,    // R    What position of the axis the CC is actually commanding measured in hundredths of an inch.
        HMI_COMMANDED_POSITION_REG_ADDR = 40013,    // W    What position of the axis the HMI is commanding measured in hundredths of an inch.
                                     //      The commanded position will not be acted upon if `IS_COMMANDED_POS_COIL_ADDR` is not set.
        JOB_PROGRESS_REG_ADDR           = 40016,    // R    The progress of the current job if there is one. Expressed as (job_progress/65536)*100%.
        JOB_START_POS_REG_ADDR          = 40019,    // R    The currently saved start position of the axis measured in hundredths of an inch.
                                     //      (While the register is read-only, the value can semantically change otherwise.)
        JOB_END_POS_REG_ADDR            = 40021,    // R    Same as JOB_START_POS_REG_ADDR but indicates the end position of the job.
        JOB_PARK_POS_REG_ADDR           = 40023,    // R    Same as JOB_START_POS_REG_ADDR but indicates the park position.
        // MIN_POS_REG_ADDR                = 40025,    // R    Minimum absolute position of the axis measured in hundredths of an inch.
        // MAX_POS_REG_ADDR                = 40026,    // R    Maximum absolute position of the axis measured in hundredths of an inch.
        JOG_SPEED_REG_ADDR              = 40032,    // R/W  target speed while jogging manually or in disengaged portion of job sequence
        PLANISH_SPEED_REG_ADDR          = 40033,    // R/W  target speed while in engaged portion of job sequence
        FAULT_CODE_REG_ADDR             = 40040,    // R    Fault code. 0 is normal operation
        // HEARTBEAT_IN_REG_ADDR           = 40050,    // W    Contains an arbitrary value set by the HMI
        // HEARTBEAT_OUT_REG_ADDR          = 40051,    // R    This should always be equal to `(heartbeat_in*2)%65536`
    };
}
